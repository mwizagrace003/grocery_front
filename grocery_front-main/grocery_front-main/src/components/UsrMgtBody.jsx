...
const UserMgtBody = () => {
  ...
  const fetchUsers = async () => {
    const response = await fetch(
      `http://localhost:8080/admin/users?pageNo=${currentPage}&pageSize=${pageSize}`
    );
  ...
  const handleDelete = async (userId) => {
    const response = await fetch(
      `http://localhost:8080/admin/users/delete/${userId}`,
      {
        method: "POST",
      }
    );
  ...
  <div className="container mt-4">
    ...
    {/* Button Group - Positioned Below Table */}
  ...
  <div className="floating-buttons">
    ...
    <Link to="/admin/search" className="btn btn-success">
